...
BASE_DIR="cpp.2025/.tls"


if ! [ "$#" -eq 2 ]; then
    echo "USAGE: <THIS_SHELL_SCRIPT> <PROBLEM_ID> <CPP_FILE_PATH>" 1>&2
    exit
fi
PROBLEM_ID="$1"
...
set +e
g++ "transformed.${EXT}" 2> "compile-error.txt"
if ! [ $? -eq 0 ]; then
    CASE_RESULT="C"
    echo "C" > "result.txt"
else
    timeout "${EXEC_TIME}" ./"a.out" < "input.txt" > "output.txt" 2> "error.txt"
    if ! [ $? -eq 0 ]; then
        CASE_RESULT="R"
        echo "R" > "result.txt"
    else
...
echo -e "\e[1;36m### VERIFIED CODE (TRANSFORMED)\e[0;0m"
echo
cat ~/"${CASE_DIR}/transformed.${EXT}"
echo
